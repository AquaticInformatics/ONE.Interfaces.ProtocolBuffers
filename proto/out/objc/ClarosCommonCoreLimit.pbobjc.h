// Generated by the protocol buffer compiler.  DO NOT EDIT!
// source: claros_common_core_limit.proto

// This CPP symbol can be defined to use imports that match up to the framework
// imports needed when using CocoaPods.
#if !defined(GPB_USE_PROTOBUF_FRAMEWORK_IMPORTS)
 #define GPB_USE_PROTOBUF_FRAMEWORK_IMPORTS 0
#endif

#if GPB_USE_PROTOBUF_FRAMEWORK_IMPORTS
 #import <Protobuf/GPBProtocolBuffers.h>
#else
 #import "GPBProtocolBuffers.h"
#endif

#if GOOGLE_PROTOBUF_OBJC_VERSION < 30002
#error This file was generated by a newer version of protoc which is incompatible with your Protocol Buffer library sources.
#endif
#if 30002 < GOOGLE_PROTOBUF_OBJC_MIN_SUPPORTED_VERSION
#error This file was generated by an older version of protoc which is incompatible with your Protocol Buffer library sources.
#endif

// @@protoc_insertion_point(imports)

#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"

CF_EXTERN_C_BEGIN

@class Recurrence;
GPB_ENUM_FWD_DECLARE(LimitType);

NS_ASSUME_NONNULL_BEGIN

#pragma mark - ClarosCommonCoreLimitRoot

/**
 * Exposes the extension registry for this file.
 *
 * The base class provides:
 * @code
 *   + (GPBExtensionRegistry *)extensionRegistry;
 * @endcode
 * which is a @c GPBExtensionRegistry that includes all the extensions defined by
 * this file and all files that it depends on.
 **/
@interface ClarosCommonCoreLimitRoot : GPBRootObject
@end

#pragma mark - Limit

typedef GPB_ENUM(Limit_FieldNumber) {
  Limit_FieldNumber_I18NKey = 1,
  Limit_FieldNumber_LimitType = 2,
  Limit_FieldNumber_Value = 3,
  Limit_FieldNumber_UnitId = 4,
  Limit_FieldNumber_Recurrence = 5,
};

@interface Limit : GPBMessage

@property(nonatomic, readwrite, copy, null_resettable) NSString *i18NKey;

@property(nonatomic, readwrite) enum LimitType limitType;

@property(nonatomic, readwrite) double value;

@property(nonatomic, readwrite) uint32_t unitId;

@property(nonatomic, readwrite, strong, null_resettable) Recurrence *recurrence;
/** Test to see if @c recurrence has been set. */
@property(nonatomic, readwrite) BOOL hasRecurrence;

@end

/**
 * Fetches the raw value of a @c Limit's @c limitType property, even
 * if the value was not defined by the enum at the time the code was generated.
 **/
int32_t Limit_LimitType_RawValue(Limit *message);
/**
 * Sets the raw value of an @c Limit's @c limitType property, allowing
 * it to be set to a value that was not defined by the enum at the time the code
 * was generated.
 **/
void SetLimit_LimitType_RawValue(Limit *message, int32_t value);

NS_ASSUME_NONNULL_END

CF_EXTERN_C_END

#pragma clang diagnostic pop

// @@protoc_insertion_point(global_scope)
